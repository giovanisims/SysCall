...
async def formulario():
    with open("html/sign-up.html", "r", encoding="utf-8") as file:
...
async def verificar(password: str = Form(...), password_confirm: str = Form(...)):
    ...
    if password == password_confirm:
        return {"message": "As senhas coincidem!"} #tem q fazer a parte q leva pra pagina inicial quando da certo, mas é problema pra depois
    else:
        rendered_html = template.render(error_message="As senhas não coincidem!")  # aqui ele colocar a msg de erro
        return HTMLResponse(content=rendered_html)
